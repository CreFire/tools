...
var (
	...
	// ErrInvalidID is returned default when trying to unmarshal an invalid ID.
...
func readMachineID() []byte {
	...
	if err == nil && len(hid) != 0 {
		...
		if _, randErr := rand.Reader.Read(id); randErr != nil {
			panic(fmt.Errorf("xid: cannot get hostname nor generate a random number: %v; %v", err, randErr).(any))
		}
	}
	return id
}
...
func randInt() uint32 {
	...
	if _, err := rand.Reader.Read(b); err != nil {
		panic(fmt.Errorf("xid: cannot generate random number: %v", err).(any))
	}
...
// Bytes Byte returns the byte array representation of `ID`
...
func FromBytes(b []byte) (ID, error) {
	var id ID
	if len(b) != rawLen {
		return id, errors.New("xid: invalid ID")
	}
	copy(id[:], b)
	return id, nil
}
